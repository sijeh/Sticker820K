...
from sticker_clip.clip import _tokenizer
from sticker_clip.clip.configuration_bert import BertConfig
from sticker_clip.clip.modeling_bert import BertModel
...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x,with_proj=True):
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask_ratio: float = 0.0, with_proj=True):
        ...
        if self.proj is not None and with_proj:
            x = x @ self.proj
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, mask_ratio=0, num_frame=3, with_proj=True):
        B, C, H, W = image.shape
        ...
        if isinstance(self.visual, ModifiedResNet):
            ...
            feature = self.visual(image.type(self.dtype), with_proj=with_proj)
        else:
            feature = self.visual(image.type(self.dtype), mask_ratio, with_proj=with_proj)

        feature = feature.view(B, num_frame, -1).mean(dim=1)
        return feature

    def encode_text(self, text, with_proj=True):
