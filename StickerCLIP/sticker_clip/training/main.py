...
# from cn_clip.training.sticker_data import get_trainval_data
from cn_clip.training.emoji_dataV2 import get_data
...
def main():
    ...
    #       torch.cuda.memory_allocated() / 1e9, 'Memory cached: ',
    #       torch.cuda.memory_cached() / 1e9, 'Max memory allocated: ',
...
# freeze bn running mean and variance
    ...
    # data = get_trainval_data(args, splits=args.splits, epoch_id=0, max_txt_length=args.context_length)
    ...
    if args.resume is not None:
        if os.path.isfile(args.resume):
            ...
            if not args.reset_data_offset:
                ...
                # data = get_trainval_data(args, splits=args.splits, epoch_id=start_epoch, max_txt_length=args.context_length)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if "valid" in data and args.valid_epoch_interval is not None and ((epoch + 1) % args.valid_epoch_interval) == 0:
        ...
        if epoch + 1 < args.max_epochs:
            ...
            # data = get_trainval_data(args, splits=args.splits, epoch_id=epoch + 1, max_txt_length=args.context_length)

        # Saving checkpoints.
        if args.should_save and num_steps_this_epoch > 0:
            if (epoch + 1) == args.max_epochs or (args.save_epoch_frequency > 0 and
                ...
                logging.info("Saved checkpoint {} (epoch {} @ {} steps) (writing took {} seconds)".format(
                    save_path, epoch + 1, steps,
                    time.time() - t1))
