...
def freeze_vision_bn(args, model):
    ...
    if 'RN' in args.vision_model:
        RN_visual_modules = model.module.visual.modules() if isinstance(
            model, nn.parallel.DistributedDataParallel) else model.visual.modules()
...
def train(model, data, epoch, optimizer, scaler, scheduler, args, global_trained_steps):
    ...
    for i in range(global_trained_steps - num_batches_per_epoch * epoch, num_batches_per_epoch):
        ...
        if is_master(args) and ((step + 1) % args.log_interval) == 0:
            ...
            logging.info(f"Global Steps: {step + 1}/{args.max_steps} | " +
                         ...
                         f"Data Time: {data_time:.3f}s | " + f"Batch Time: {batch_time:.3f}s | " +
                         f"LR: {optimizer.param_groups[0]['lr']:5f} | " + f"logit_scale: {m.logit_scale.data:.3f} | " +
                         f"Global Batch Size: {len(images) * args.world_size}")

        # if args.data_root is not None and args.valid_step_interval is not None and ((step + 1) % args.valid_step_interval) == 0:
        if args.val_data is not None and args.valid_step_interval is not None and ((step + 1) % args.valid_step_interval) == 0:
            assert "valid" in data, "Error: Valid dataset has not been built."
...
def evaluate(model, data, epoch, args, steps):
    ...
    with torch.no_grad():
        ...
        logging.info(f"Validation Result (epoch {epoch + 1} @ {steps} steps) | "
