...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, mask_ratio=0, num_frame=3):
        B, C, H, W = image.shape
        image = image.view(B * num_frame, C // num_frame, H, W)
        if isinstance(self.visual, ModifiedResNet):
            # mask_ratio > 0 (FLIP strategy) is currently only implemented for VisualTransformer.
            feature = self.visual(image.type(self.dtype))
        else:
            feature = self.visual(image.type(self.dtype), mask_ratio)

        feature = feature.view(B, num_frame, -1).mean(dim=1)
        return feature
    ...
    def forward(self, image, text, mask_ratio=0, num_frame=3):
        assert image is not None or text is not None, "text and image cannot both be None!"

        if image is None:
            return self.encode_text(text)
        elif text is None:
            return self.encode_image(image, num_frame=num_frame)
        image_features = self.encode_image(image, mask_ratio, num_frame=num_frame)
...
class CLIPVisualEncoder(nn.Module):
    ...
    def forward(self, image, mask_ratio=0, num_frame=3):
        # assert image is not None or text is not None, "text and image cannot both be None!"
    ...
    #     return logits_per_image, logits_per_text
    @classmethod
    def from_pretrained(cls, model_name, ckpt_path):
        cfg_path = Path(__file__).parent / f"model_configs/{model_name.replace('/', '-')}.json"

        assert os.path.exists(cfg_path), cfg_path

        with open(cfg_path, 'r') as f:
            model_info = json.load(f)
            if isinstance(model_info['vision_layers'], str):
                model_info['vision_layers'] = eval(model_info['vision_layers'])
        logger.info('Start initialize CLIPVisualEncoder with config: \n', json.dumps(model_info, indent=4))
        model = cls(**model_info).cpu()
        model.config = OmegaConf.create(model_info)

        assert os.path.exists(ckpt_path), "The checkpoint file {} not exists!".format(ckpt_path)

        checkpoint = torch.load(ckpt_path, map_location='cpu')
        sd = checkpoint['state_dict']
        if next(iter(sd.items()))[0].startswith('module'):
            sd = {k[len('module.'):]: v for k, v in sd.items() if "bert.pooler" not in k}
        logger.info('Start loading ckpt for CLIPVisualEncoder')
        missing_keys, unexpected_keys = model.load_state_dict(sd, strict=False)
        logger.info("Missing keys: ", json.dumps(missing_keys))
        logger.info('Unexcepted Keys: ', json.dumps(unexpected_keys))
