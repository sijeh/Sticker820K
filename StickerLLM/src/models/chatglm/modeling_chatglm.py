...
def attention_fn(
    ...
    # matmul: [b * np, sq, hn]
    # print('shape: ', attention_probs.shape, value_layer.shape)
...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.rotary_emb = RotaryEmbedding(
            self.hidden_size // (self.num_attention_heads * 2) if position_encoding_2d else self.hidden_size //
    ...
    def forward(
        ...
        if self.position_encoding_2d:
            ...
            # print('diff: ', position_ids.max() + 1, q1.shape[0])
            # cos, sin = self.rotary_emb(q1, seq_dim=0)
            ...
            # cos, sin = self.rotary_emb(value_layer, seq_dim=0)
            # [seq_len, batch, num_attention_heads, hidden_size_per_attention_head]
            query_layer, key_layer = apply_rotary_pos_emb_index(query_layer, key_layer, cos, sin, position_ids)

        # [seq_len, batch, hidden_size]
        context_layer, present, attention_probs = attention_fn(self=self,
        ...
        if self.position_encoding_2d:
            ...
            block_position_ids = torch.cat(
                (torch.zeros(seq_length, dtype=torch.long,
                             device=device), torch.arange(context_length - seq_length, dtype=torch.long, device=device) + 1))
    ...
    def forward(
        ...
        if past_key_values is None:
            ...
            if attention_mask is None:
                ...
                attention_mask = self.get_masks(seq=seq, device=input_ids.device)
            ...
            if position_ids is None:
                ...
                position_ids = self.get_position_ids(seq=seq,
        ...
        if attention_mask is None:
            ...
            attention_mask = attention_mask.to(input_ids.device) if input_ids is not None else attention_mask.to(inputs_embeds.device)
        ...
        for i, layer in enumerate(self.layers):
            ...
            #                   use_cache=use_cache,
            #                   output_attentions=output_attentions)
            layer_ret = checkpoint(layer, hidden_states, position_ids, attention_mask, torch.tensor(i), past_key_values[i],
            ...
            #                use_cache=use_cache,
            #                output_attentions=output_attentions)
            # layer_ret = checkpointing.checkpoint(func, hidden_states)
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.lm_head = skip_init(nn.Linear, config.hidden_size, config.vocab_size, bias=False, dtype=torch.half)
    ...
    def prepare_inputs_for_generation(self,
        ...
        if past is not None or past_key_values is not None:
            ...
            attention_mask, position_ids = self.get_masks_and_position_ids(seq=seq,
                                                                           mask_position=mask_position,
    ...
    def chat(self,
        ...
        # logits_processor.append(RetrievalProbLogitsProcessor(150344, 0.0))
        logits_processor.append(RetrievalEndLogitsProcessor(150344, 150345))
        if stopping_criteria is None:
            stopping_criteria = StoppingCriteriaList()
            # stopping_criteria.append(RetrievalStoppingCriteria(150344))
        ...
        generated_ids = output_ids[len(input_ids.input_ids[0]):]
