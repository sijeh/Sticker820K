...
class SPTokenizer:
    ...
    def _configure_tokenizer(
        ...
        # whitespaces
        for token in [SPTokenizer.get_tab_token()] + [SPTokenizer.get_blank_token(i) for i in range(2, max_blank_length + 1)]:
    ...
    def _build_text_tokenizer(self, encode_special_tokens=False):
        ...
        self._configure_tokenizer(tokenizer, self.special_tokens, self.max_blank_length, self.byte_fallback,
    ...
    def encode(self, text: str, linebreak=True, whitespaces=False, special_tokens=True, add_dummy_prefix=False) -> List[int]:
        """
    ...
    def decode(self, text_ids: List[int], special_tokens=True) -> str:
    ...
    def tokenize(self, text: str, linebreak=True, whitespaces=False, special_tokens=True, add_dummy_prefix=False) -> List[str]:
        """
        @param text: Text to encode.
        @param linebreak: Whether to encode newline (\n) in text.
        @param whitespaces: Whether to encode multiple whitespaces or tab in text, useful for source code encoding.
...
class ChatGLMTokenizer(PreTrainedTokenizer):
    ...
    def add_tokens(self, new_tokens: Union[str, AddedToken, List[Union[str, AddedToken]]], special_tokens: bool = False) -> int:
        added_len = super().add_tokens(new_tokens, special_tokens)

        # if isinstance(new_tokens, str):
        #     print('add new token to sp tokenizer')
        #     self.sp_tokenizer.text_tokenizer.proto.pieces.append(
        #         sp_model.ModelProto.SentencePiece(piece=new_tokens, score=0.0, type=4))
        #     self.sp_tokenizer.special_text_tokenizer.proto.pieces.append(
        #         sp_model.ModelProto.SentencePiece(piece=new_tokens, score=0.0, type=4))
        #     self.sp_tokenizer.text_tokenizer.refresh()
        #     self.sp_tokenizer.special_text_tokenizer.refresh()
        if added_len > 0:
            if isinstance(new_tokens, str):
                # self.sp_tokenizer.text_tokenizer.add_special_tokens([new_tokens])
                self.sp_tokenizer.special_text_tokenizer.add_special_tokens([new_tokens])
