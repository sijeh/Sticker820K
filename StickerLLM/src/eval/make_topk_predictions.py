...
def parse_args():
    ...
    parser.add_argument('--text-feats', type=str, required=True, help="Specify the path of llm features.")
    ...
    with open(args.output, "w") as fout:
        with open(args.text_feats, "r") as fin:
            for line in tqdm(fin):
                ...
                score_tuples = []
                text_feat_tensor = torch.tensor([text_feat], dtype=torch.float).cuda(args.gpu)  # [1, feature_dim]
                idx = 0
                while idx < len(image_ids):
                    img_feats_tensor = torch.from_numpy(
                        image_feats_array[idx:min(idx +
                                                  args.eval_batch_size, len(image_ids))]).cuda(args.gpu)  # [batch_size, feature_dim]
                    batch_scores = text_feat_tensor @ img_feats_tensor.t()  # [1, batch_size]
                    for image_id, score in zip(image_ids[idx:min(idx + args.eval_batch_size, len(image_ids))],
